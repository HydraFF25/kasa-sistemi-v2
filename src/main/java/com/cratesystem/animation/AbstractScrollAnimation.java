...
public abstract class AbstractScrollAnimation implements CrateAnimation {
    ...
    public void play(CratePlugin plugin, Player player, Crate crate, CrateReward reward) {
        ...
        Inventory inv = Bukkit.createInventory(holder, SIZE, ColorUtils.color(getTitlePrefix() + " " + crate.getDisplayName()));
        ...
        runFrame(plugin, player, inv, strip, 0, TOTAL_FRAMES, winIndex - TRAVEL, winIndex, crate, reward);
    }

    private void runFrame(CratePlugin plugin, Player player, Inventory inv, List<CrateReward> strip,
                           int frame, int totalFrames, int startIndex, int winIndex, Crate crate, CrateReward reward) {
        ...
        if (frame > totalFrames) {
            finish(plugin, player, inv, crate, reward);
            return;
        }
        ...
        Bukkit.getScheduler().runTaskLater(plugin, () ->
                runFrame(plugin, player, inv, strip, nextFrame, totalFrames, startIndex, winIndex, crate, reward), delay);
    }
    ...
    private void finish(CratePlugin plugin, Player player, Inventory inv, Crate crate, CrateReward reward) {
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
